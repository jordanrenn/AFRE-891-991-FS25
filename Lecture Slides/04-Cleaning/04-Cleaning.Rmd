...
```{r}
#Need to add the _all to str_replace so that it continnues looking after finding the first match
#Specifying quote, need to escape with /
heights2 <- reported_heights %>%
...
```{r}
#This is trying to filter the original data to show what we are trying to fix
heights_pun <- filter(heights2,
                      str_detect(height, "\\."))
view(heights_pun)
